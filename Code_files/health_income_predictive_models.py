...
# map income level into groups like like low medim  high
def map_income_groups(val):
	try:
		v = float(val)
	except:
		return "Don't know/Not sure/Missing"
	if v in [1, 1.0, 2, 2.0, 3, 3.0, 4, 4.0]:
		return "Low Income"
	elif v in [5, 5.0, 6, 6.0, 7, 7.0]:
		return "Middle Income"
	elif v in [8, 8.0 , 9 , 9.0, 10 ,10.0 ,11 ,11.0]:
		return "High Income"
	else:
		return np.nan


df['Income_Group'] = df['Income_Categories'].apply(map_income_groups)
...
#Chi-square test to see if income_group and health status are related
df = df.dropna(subset=['Income_Group'])
ct = pd.crosstab(df['Income_Group'], df['General_Health_Status'])
...
# prop.plot(kind="bar", stacked=True, figsize=(9,5))
# plt.title("Health Status Distribution by Income Group")
...
# From the p- value we can can see that there they is a statisticaly  difference between income an health status

# Smart Question : Do exercise and BMI explain the relationship between income and health?
df_q2 = df[['Income_Group', 'General_Health_Status', 'Exercise_Past_30_Days', 'BMI_Value']].copy()
print(df_q2.Income_Group.describe(), '\n', df_q2.Income_Group.value_counts(dropna=False))
print(df_q2.General_Health_Status.describe(), '\n', df_q2.General_Health_Status.value_counts(dropna=False))
print(df_q2.Exercise_Past_30_Days.describe(), '\n', df_q2.Exercise_Past_30_Days.value_counts(dropna=False))
print(df_q2.BMI_Value.describe(), '\n', df_q2.BMI_Value.value_counts(dropna=False))

def recode_health(x):
	if isinstance(x,str):
		x = x.strip().lower()
		if x in ['excellent' ,'very good' , 'good']:
			return 1 
		elif x in ['fair', 'poor']:
			return 0
	return np.nan

df_q2['GoodHealth'] = df_q2['General_Health_Status'].apply(recode_health)
df_q2['Exercise'] = df_q2['Exercise_Past_30_Days'].map({'Yes':1, 'No':0})
df_q2 = df_q2[(df_q2['BMI_Value'] > 14) & (df_q2['BMI_Value'] < 60)]
df_q2.head()

import statsmodels.formula.api as smf
# Model1  Health - Income only
model1 = smf.logit("GoodHealth ~ C(Income_Group)", data=df_q2).fit()
print(model1.summary())

#  Model 2: Add Exercise & BMI
model2 = smf.logit("GoodHealth ~ C(Income_Group) + Exercise + BMI_Value", data=df_q2).fit()
...
# Smart Question : Can we predict that someone report poor health using income, age and lifestyle factors?

# Make a copy from df_q2 and add Age
df_q3 = df_q2.copy()
df_q3['Age'] = df['Age']   

# Drop rows with missing values
df_q3 = df_q3.dropna(subset=['GoodHealth', 'Income_Group', 'Exercise', 'BMI_Value', 'Age'])

df_q3.head()

from sklearn.model_selection import train_test_split

X = pd.get_dummies(df_q3[['Income_Group', 'Exercise', 'BMI_Value', 'Age']], drop_first=True)
y = df_q3['GoodHealth']
...
#Confusion Matrix
def confusion_table(y_true, y_pred, title="Confusion Matrix"):
    cm = confusion_matrix(y_true, y_pred)
    table = pd.DataFrame(
        cm,
        index=["Actual Poor (0)", "Actual Good (1)"],
        columns=["Pred Poor (0)", "Pred Good (1)"]
    )
    print(f"\n===== {title} =====")
    print(table)


# Logistic Regeresion model 
...
confusion_table(y_test, y_pred_log, "Logistic Regression")
...
confusion_table(y_test, y_pred_tree, "Decision Tree")
...
confusion_table(y_test, y_pred_rf, "Random Forest")
